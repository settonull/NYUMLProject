...
def get_Teams(scores): #list of teams and conferences for given year
    ...
    for pos in ['Home','Vis']: #add conferences
        temp = scores[['{}ID'.format(pos),'{}Conf'.format(pos)]]
    ...
    stats['Conf'] = stats['HomeConf'].fillna(stats['VisConf']) #merge conf features
    stats.drop(['VisConf','HomeConf'], axis=1, inplace=True) #drop old conf features
...
def endYear_ptsWinsGamesPyth(team, ind, final_game, exp):
...
def getFinal_ptsWinsGamesPyth(scores, exp): #wrapper for getting final stats
    ...
    stats[ind] = stats.index.to_series().apply(lambda x: endYear_ptsWinsGamesPyth(x, ind, scores.loc[(scores['HomeID'] == x) | (scores['VisID'] == x)].tail(1), exp)) #fill features
    return stats
...
def add_pythWins(scores, exp, prev_scores=None):
    ...
    if prev_scores is not None: #prev luck
        prev_stats = getFinal_ptsWinsGamesPyth(prev_scores, exp) #get 
...
for j, year in enumerate(range(2013, 2017)):
    ...
    data_list[j] = pd.read_csv('data/scores/scores{}.csv'.format(year))
    data_list[j] = add_ptsWinsGames(data_list[j])
    if year == 2013:
...
for j, year in enumerate(range(2013, 2017)):
    ...
    data_list[j].to_csv('data/ptsWinsGames_pyth_elo/data_pwg_pyth_elo_{}.csv'.format(year), index=False)