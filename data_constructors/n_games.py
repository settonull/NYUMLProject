...
def ngames_summarize(dataframe, home_features, away_features, gen_features):
    ...
    for idx, game in dataframe.iterrows():
        ...
        cg_d1_match = game['D1_Match']
        cg_season = game['Season']

        # Build historical game stats for home and away teams separately
        ht_h_games = dataframe[(dataframe['HomeID']==homeID) & (dataframe['DateTime'] < cg_datetime)][home_features + ['VisFinal','margin','Season','DateTime']]
        ht_h_games.columns = ['HH_' + x if x not in ['Season','DateTime'] else x for x in ht_h_games.columns]
        ht_a_games = dataframe[(dataframe['AwayID']==homeID) & (dataframe['DateTime'] < cg_datetime)][away_features + ['VisFinal','margin','Season','DateTime']]
        ht_a_games.columns = ['HA_' + x if x not in ['Season','DateTime'] else x for x in ht_a_games.columns]
        vt_h_games = dataframe[(dataframe['HomeID']==awayID) & (dataframe['DateTime'] < cg_datetime)][home_features + ['HomeFinal','margin','Season','DateTime']]
        vt_h_games.columns = ['VH_' + x if x not in ['Season','DateTime'] else x for x in vt_h_games.columns]
        vt_a_games = dataframe[(dataframe['AwayID']==awayID) & (dataframe['DateTime'] < cg_datetime)][away_features + ['HomeFinal','margin','Season','DateTime']]
        vt_a_games.columns = ['VA_' + x if x not in ['Season','DateTime'] else x for x in vt_a_games.columns]

        # Apply summary functions to historical data
        n_games = 5 # All results need to have the same shape. Might need to build handling for < 10 games
        n_game_summaries_df = ngames_summaries([ht_h_games, ht_a_games, vt_h_games, vt_a_games], n_games)
        in_season_summaries_df = in_season_summary([ht_h_games, ht_a_games, vt_h_games, vt_a_games], cg_season)
        summaries_df = pd.concat([n_game_summaries_df, in_season_summaries_df], axis=1)

        # Combine historical game stats for the current game
        summaries_df['D1_Match'] = cg_d1_match
        summaries_df['target_margin'] = cg_margin
        summaries_df['teams_key'] = "-".join([str(homeID), str(awayID)])
        summaries_df['DateTime'] = str(cg_datetime)
        summaries_df['Season'] = cg_season
        summaries_df = summaries_df.set_index(['teams_key', 'DateTime', 'Season'])
